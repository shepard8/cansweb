...
clean:
	...
	-rm -f static/*.{png,tex,tikz,dot,svg}
	-rm -f *.{aux,log}
...
full:
	sudo true
	cd /home/shepard/ocamlcanswer/console-oriented && ocaml setup.ml -build && sudo ocaml setup.ml -uninstall && sudo rm -rf ~/.opam/system/lib/canswer* && sudo ocaml setup.ml -install
	make test.opt
